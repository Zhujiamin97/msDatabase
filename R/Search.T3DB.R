#'@title Search T3DB Database
#'
#'@author JiaMin ZHU
#'@description
#' "http://www.t3db.ca/toxins/"
#' 
#'@param T3DB_ID  "T3D3216"
#'@param Exact_Mass
#'@param mz_error
...
#'
#'@example 
#' use name seacrh : search.t3db(T3DB_ID = "T3D3216")

search.t3db <-function(T3DB_ID = "T3D3216",
                       Exact_Mass = NULL,
                       mz_ppm = 5,
                       ms2.mz.tol = 0.02,
                       mz.tol.type.ms2 = "Da",
  ...
  if(is.numeric(Exact_Mass)&is.numeric(mz_ppm)){
    ...
    for (page in 1:10000) {
      ...
      mass_url <- paste0("http://www.t3db.ca/structures/search/small_molecules/mass?",
                         "&page=",page,
      ...
      str_ids = grep("/toxins/T3D",news,value = TRUE,fixed = TRUE)
      if(length(str_ids) == 0){
        break
      }
      ids <- lapply(str_ids, function(x){
        strsplit(x,"/toxins/")[[1]][2]
      }) %>% unlist()
      t3db_id <- c(t3db_id,ids)
    }
    
  }else if(!is.null(T3DB_ID)){
    t3db_id <- T3DB_ID
  }
  message("The number of results: ",length(t3db_id))
  # after getting hmdb ids
  id_url <- paste0(url,t3db_id,"#spectra")
  spec_ids <- 
  ...
  #-------get ms2---------#
  ms2_urls <- paste0("http://www.t3db.ca",spec_ids)
  ...
  ms2_spectra <- lapply(ms2_urls, function(temp_id) {
    ...
    ms1_info = data.frame(T3DB_ID = ms1_info$T3DB_ID,
